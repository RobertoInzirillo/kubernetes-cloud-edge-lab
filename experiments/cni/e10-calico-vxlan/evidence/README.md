...
E10. Tutti i file elencati sono copie byte-identiche; questo README e
...
| `service-kube-proxy-attribution-corrected.log` | output di attribuzione | Catene `KUBE-SVC`/`KUBE-SEP`, DNAT e delta mirato dei contatori attribuiscono i flussi Service a kube-proxy iptables. |
...
La cattura e il log HTTP appartengono allo stesso GET controllato. La selezione
non include il controllo preliminare del Service, successivamente sostituito
da una verifica più mirata, né rendering Helm e inventari duplicati. I
riferimenti runtime, inclusi sandbox dei Pod, PID, IP, veth e nomi delle
catene, possono differire in una nuova replica.
